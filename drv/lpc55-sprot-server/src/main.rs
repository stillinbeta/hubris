...
fn main() -> ! {
    ...
    ringbuf_entry!(Trace::Line); // XXX last entry is here
    ...
    loop {
        ...
        transmit = match server.handler.handle(
            ...
            Some(_txlen) => {
                ringbuf_entry!(Trace::TxQueued);
                true
            }
            None => {
                ringbuf_entry!(Trace::TxNone);
                false
            }
        }
    }
