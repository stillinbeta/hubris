...
use zerocopy::{byteorder, AsBytes, FromBytes, Unaligned, U32, U16};
...
impl Transceivers {
    ...
    pub fn get_presence(&self) -> Result<u32, FpgaError> {
        let f0: u16 = u16::from_be(self.fpgas[0].read(Addr::QSFP_STATUS_PRESENT_L)?);
        let f1: u16 = u16::from_be(self.fpgas[1].read(Addr::QSFP_STATUS_PRESENT_L)?);
    ...
    pub fn get_power_good_timeout(&self) -> Result<u32, FpgaError> {
        let f0: u16 = u16::from_be(self.fpgas[0].read(Addr::QSFP_STATUS_PG_TIMEOUT_L)?);
        let f1: u16 = u16::from_be(self.fpgas[1].read(Addr::QSFP_STATUS_PG_TIMEOUT_L)?);
        Ok((f1 as u32) << 16 | (f0 as u32))
    }

    pub fn get_irq_rxlos(&self) -> Result<u32, FpgaError> {
        let f0: u16 = u16::from_be(self.fpgas[0].read(Addr::QSFP_STATUS_IRQ_L)?);
        let f1: u16 = u16::from_be(self.fpgas[1].read(Addr::QSFP_STATUS_IRQ_L)?);
        Ok((f1 as u32) << 16 | (f0 as u32))
    }

    pub fn setup_i2c_read(&self, reg: u8, num_bytes: u8, port_bcast_mask: u32) -> Result<(), FpgaError> {
        let bcast_mask: U32<byteorder::BigEndian> = U32::new(port_bcast_mask);
        let f0_request = TransceiversI2CRequest {
            reg,
            num_bytes,
            port_bcast_mask: U16::new(bcast_mask.get() as u16),
            op: ((TransceiverI2COperation::RandomRead as u8) << 1) | Reg::QSFP::I2C_CTRL::START,
        };
        self.fpgas[0].write(WriteOp::Write, Addr::QSFP_I2C_REG_ADDR, f0_request)?;

        let f1_request = TransceiversI2CRequest {
            reg,
            num_bytes,
            port_bcast_mask: U16::new((bcast_mask.get() >> 16) as u16),
            op: ((TransceiverI2COperation::RandomRead as u8) << 1) | Reg::QSFP::I2C_CTRL::START,
        };
        self.fpgas[1].write(WriteOp::Write, Addr::QSFP_I2C_REG_ADDR, f1_request)?;

        Ok(())
    }

    // pub fn get_i2c_read_buffer(&self, u8: port, u8: numbytes) -> Result<[u8; 128], FpgaError> {
    //     if port < 16 {
    //         let 
    //     }
    // }
